...
export function NewNoteCard() {
  ...
  function handleStartEditor() {
    shouldShowOnBoarding ? setShouldShowOnBoarding(false) : setShouldShowOnBoarding(true);
  }
  ...
  return (
    <Dialog.Root>
      <Dialog.Trigger className="rounded-md bg-slate-700 p-5 gap3 flex flex-col text-left hover:ring-2 outline-none hover:ring-slate-600 focus-visible:ring-2 focus-visible:ring-lime-400">
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 max-w-[640px] w-full h-[60vh] bg-slate-700 rounded-md flex flex-col outline-none overflow-hidden">
          <Dialog.Close onClick={handleStartEditor} className="absolute right-0 top-0 bg-slate-800 p-1.5 text-slate-400 hover:text-slate-100">
            <X className="size-5" />
          </Dialog.Close>
            ...
            <div className="flex flex-1 flex-col gap-3 p-5">
              ...
              {shouldShowOnBoarding ? (
                <p className="text-sm leading-6 text-slate-400">
                  Comece <button className="font-medium text-lime-400 hover:underline">gravando uma nota</button> ou áudio ou se preferir{" "}
